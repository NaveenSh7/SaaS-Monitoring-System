...
router.post('/' , async (req,res)=>{
     try {
        const {api_id,status,started_at,ended_at} = req.body;

            if (!api_id || !status || !started_at) {
      return res.status(400).json({ message: 'All fields required' });
    }
        const response = await createUptime(api_id,status,started_at,ended_at);
        res.json(response);
...
// changing status and adding new uptime

router.put('/' , async (req,res)=>{
  try {
    const {api_id,status} = req.body;
    ...
    if(status !== ongoing.status)
    {
        ...
        const result2 = await createUptime(api_id ,status);
